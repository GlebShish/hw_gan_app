...
def load_condfile(fname, skiprows=5):
    ...
    ii, jj, vals = data.T
    ij = np.asarray((ii, jj), dtype=int).T
    return ij, vals


def medfilt_plot(ax, y, x=None, start=500, filter_width=101, symlog=True, **kws):
    if x is None:
        x = range(len(y))[start:][int(filter_width / 2):-int(filter_width / 2)]
    ax.plot(x, signal.medfilt(y[start:], filter_width)[int(filter_width / 2):-int(filter_width / 2)], **kws)
    if symlog:
...
def scatter_ij(ax, ij, vals):
    ij0, ij1 = ij[np.where(vals == -1)], ij[np.where(vals == 1)]
    ax.scatter(ij0[:, 1], ij0[:, 0], marker='x', s=20, c='C1')
    ax.scatter(ij1[:, 1], ij1[:, 0], marker='o', s=20, c='C0')


class Plotter(object):
    def __init__(self, outdir, netG, netI):
        self.outdir = outdir
        self.netGI = NetGI(netG, netI)


    #def flush(self, iteration, ij, val, w_plots, w_picture):
    #    self.netGI.netI.eval()
    #    with torch.no_grad():
    #        x = self.netGI(w_plots, w_picture).detach().cpu().numpy().squeeze()
    #    self.netGI.netI.train()
    def flush(self, iteration, x, ij, val):

        ncols = 8
        nrows = int(len(x) / ncols)
        fig, axs = plt.subplots(nrows, ncols, figsize=(10, int(10 * nrows / ncols)))
        fig.subplots_adjust(top=1, right=1, bottom=0, left=0, hspace=.05, wspace=.05)
        fill_imgs(axs, x)
        for ax in axs.flat:
            scatter_ij(ax, ij, val)
        fig.canvas.draw()
