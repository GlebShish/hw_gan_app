...
class FC_selu_first(nn.Module):
    ...
    def __init__(self, input_size=1, output_size=1, hidden_layer_size=512, num_extra_layers=2):
        super(FC_selu_first, self).__init__()
        ...
        for i in range(num_extra_layers):
            main.add_module('layer-{}'.format(i + 1),
                            nn.Sequential(nn.Linear(hidden_layer_size, hidden_layer_size, bias=False), SELU()))
        main.add_module('layer-{}'.format(num_extra_layers + 1),
                        nn.Sequential(nn.Linear(hidden_layer_size, output_size, bias=True)))
...
class FC_selu(nn.Module):
    ...
    def __init__(self, input_size=1, output_size=1, hidden_layer_size=512, num_extra_layers=2):
        ...
        extra = nn.Sequential()
        extra.add_module('conv-0', nn.Sequential(nn.Conv2d(1, 32, 5), nn.Tanh(), nn.AvgPool2d(2)))
        extra.add_module('conv-1', nn.Sequential(nn.Conv2d(32, 16, 5), nn.Tanh(), nn.AvgPool2d(2), Flatten()))
        extra.add_module('dense', nn.Sequential(nn.Linear(2704, 30, bias=True)))
        ...
        self.main = main
        self.extra = extra
        self.last = nn.Linear(60, 30, bias=True)

        init_linear(self.last)

    def forward(self, x, picture):
        main = self.main(x)
        extra = self.extra(picture)
        return self.last(torch.cat((main, extra), 1))
